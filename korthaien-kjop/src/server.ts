...
import { db, migrate, hentSettings, settSetting, CONDITIONS, type Settings } from "./db.js";
import { søk, løsBulk } from "./catalog.js";
import { parseBulk, MAX_LINJER } from "./bulk.js";
import { lagOrdre, hentOrdre, utløpGamleOrdrer, HttpFeil } from "./orders.js";
import { hentSetRule } from "./pricing.js";
import { importerScryfall } from "./import-scryfall.js";
import { synkMystore, harMystore } from "./mystore.js";
...
app.get("/api/orders/:orderNo", fang(async (req: any, res: any) => {
  ...
  const { email, phone, admin_note, ...trygt } = o as any;
  res.json(trygt);
}));
...
// Justering av linjer ved mottak: antall og pris kan avvike fra det kunden
// oppga, typisk fordi condition ikke stemte.
app.patch("/api/admin/lines/:id", krevAdmin, fang(async (req: any, res: any) => {
  const id = Number(req.params.id);
  const { qty, unit_nok, qty_received, condition } = req.body || {};
  const felt: string[] = [];
  const args: any[] = [];
  for (const [navn, verdi] of [["qty", qty], ["unit_nok", unit_nok], ["qty_received", qty_received]] as const) {
    if (verdi !== undefined) {
  ...
  if (!felt.length) throw new HttpFeil(400, "Ingenting å endre");
  args.push(id);
...
async function oppdaterTotal(orderId: number): Promise<void> {
  await db().execute({
    sql: `UPDATE orders SET total_nok =
            COALESCE((SELECT SUM(unit_nok * COALESCE(qty_received, qty)) FROM order_lines WHERE order_id = ?), 0)
          WHERE id = ?`,
...
app.get("/api/admin/sets", krevAdmin, fang(async (_req: any, res: any) => {
  ...
  const r = await db().execute(`
    SELECT s.code, s.name, s.released_at, s.card_count,
           r.enabled, r.wanted_default, r.conditions, r.ladder
...
app.put("/api/admin/sets/:code", krevAdmin, fang(async (req: any, res: any) => {
  ...
  const { enabled, wanted_default, conditions, ladder } = req.body || {};
  ...
  await db().execute({
    sql: `INSERT INTO set_rules (set_code, enabled, wanted_default, conditions, ladder, updated_at)
          VALUES (?, ?, ?, ?, ?, ?)
...
app.put("/api/admin/settings", krevAdmin, fang(async (req: any, res: any) => {
  const lov: (keyof Settings)[] = [
    "usd_nok", "buy_pct", "min_buy_nok", "default_conditions",
...
migrate()
  .then(() => {
    app.listen(PORT, () => console.log(`Korthaien Kjøp lytter på :${PORT}`));
    // Rydder reservasjoner hver time, så kvoter ikke blir stående låst.
    setInterval(() => utløpGamleOrdrer().catch(console.error), 3600_000);
  })
  .catch((e) => {
