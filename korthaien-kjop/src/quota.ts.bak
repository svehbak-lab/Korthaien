...
async function hentØnsket(cardId: string, finish: string, regel: SetRule) {
  ...
  // Foil arves ikke fra settets standardantall. Foil er en annen vare, og
  // å arve tallet ville fylt hyllene med foils du ikke har bedt om.
  if (finish === "foil") return { wanted: 0, kilde: "av" as const };
...
export function regnLedig(
  ...
  if (oppslag && oppslag.want !== null) {
    wanted = oppslag.want;
    kilde = "kort";
  } else if (!regel.enabled || finish === "foil") {
    wanted = 0;
    kilde = "av";
  } else {
