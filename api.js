...
// Bytte av utgave er å flytte antallet over på et annet trykk. Kvoten på det
// nye kortet gjelder, så kunden kan ende med færre enn hen hadde.
export function byttTilbud(kurv, k, nyttTilbud) {
  const linje = kurv[k];
  if (!linje || nyttTilbud.card_id === linje.tilbud.card_id) return kurv;
  const { [k]: _, ...uten } = kurv;
  // Tilstanden følger med hvis det nye settet tar imot den.
  const cond = nyttTilbud.conditions.some((c) => c.condition === linje.condition)
    ? linje.condition
    : nyttTilbud.conditions[0]?.condition;
  if (!cond) return kurv;
  return leggTil(uten, nyttTilbud, cond, linje.qty);
}
